...
if start_round == 1:
    ...
    memory_usage_df = pd.DataFrame(columns=['round', 'client', 'cpu_memory_usage', 'gpu_memory_usage'])
    weights_memory = []
...
for round_number in range(start_round, end_round):
    ...
    for i in range(5):
        ...
        pes_parametres_client = pickle.dumps(weights)
        round_weights[f'Client{client_number}_MB'] = sys.getsizeof(pes_parametres_client) / (1024 ** 2)
    ...
    def calculate_memory_of_trainable_layers(model):
        total_memory = 0
        for layer in model.layers:
            if layer.trainable:
                weights = layer.get_weights()
                for weight in weights:
                    total_memory += weight.size * weight.itemsize
    ...
    weights_bytes = calculate_memory_of_trainable_layers(global_model)

    round_weights["Funcio_nova_MB"] = weights_bytes
