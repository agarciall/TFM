...
def generate_validation_images(num_rounds, num_clients):
    validation_images = [[] for _ in range(num_clients)]  # Initialize a list of empty lists
                ...
                if os.path.isdir(class_dir_path):
                    for filename in os.listdir(class_dir_path):
                        ...
                        if os.path.isfile(img_path):
                            ...
                            label = os.path.basename(os.path.dirname(img_path))  # Assuming the label is the first part of the filename
...
def move_images(source_dir, target_dir, percentage=0.2):
    # Lista de subdirectorios esperados
    expected_subdirs = ['Basophils', 'Eosinophils', 'Erythroblasts', 'IG', 'Lymphoblasts', 'Lymphocytes', 'Monocytes', 'Neutrophils', 'Platelet']

    # Obtén una lista de todos los subdirectorios en el directorio fuente
    subdirs = [d for d in os.listdir(source_dir) if os.path.isdir(os.path.join(source_dir, d))]

    # Verifica que todos los subdirectorios esperados estén presentes
    ...
    # Para cada subdirectorio, mueve un porcentaje de imágenes al subdirectorio correspondiente en target_dir
    for subdir in subdirs:
        subdir_source = os.path.join(source_dir, subdir)
        subdir_target = os.path.join(target_dir, subdir)

        # Crea el subdirectorio en target_dir si no existe
        os.makedirs(subdir_target, exist_ok=True)

        # Obtén una lista de todos los nombres de archivos en el subdirectorio fuente
        files = os.listdir(subdir_source)

        # Calcula el número de archivos para mover
        num_files_to_move = int(percentage * len(files))

        # Selecciona aleatoriamente un subconjunto de archivos para mover
        files_to_move = random.sample(files, num_files_to_move)

        # Mueve los archivos seleccionados al subdirectorio objetivo
        for file_name in files_to_move:
            source_file = os.path.join(subdir_source, file_name)
            target_file = os.path.join(subdir_target, file_name)
            shutil.move(source_file, target_file)

        # Imprime el número de imágenes en el directorio de destino después de mover las imágenes
